...
//		@ResponseBody
//		public XoadoResult Fulltextuser(@RequestParam ("page") int page,@RequestParam("rows") int rows,HttpServletRequest request){
//			return iUser.all_search(page,rows,request);	
//		}
		/**
		 * 
		 * @param request  header中保存XOADOTOKENID
		 ...
		 * @param text
		 * @param request
		 * @return   用户全文搜索    all_search
		 */
		@RequestMapping(method=RequestMethod.GET)
		@ResponseBody
		public XoadoResult all_search(@RequestParam(value="text",required=false) String text, HttpServletRequest request) {
			XoadoResult result = iUser.Fulltextuser(text, request);
			return result;
		}
}
