...
//	从session中获取到token
		AccessIdentity token = (AccessIdentity)request.getSession().getAttribute(XoadoConstant.XOADOTOKEN);
//	token中获取XOADOTOKENID
		@SuppressWarnings("unused")
		String XOADOTOKENID = token.getXOADOTOKENID();
		String userId = token.getUserId();
		TblUserExample example = new TblUserExample();
...
//	s实名认证的信息
	...
	@Override
	public XoadoResult Fulltextuser(String text, HttpServletRequest request) {
		System.out.println("----------"+text);
		if(text == null){
			List<TblUser> list = tblUserMapper.selectAll();
			return XoadoResult.build(Integer.parseInt(BaseRetCode.CODE_SUCCESS.getRetCode()), BaseRetCode.CODE_SUCCESS.getRetMsg(),list);
		}
		String sql ="SELECT a.Phone_number,a.userId,a.name,a.Nick_name FROM tbl_user as a WHERE MATCH(Phone_number,name,Nick_name) AGAINST('+"+text+"' IN BOOLEAN MODE)";
		List<LinkedHashMap<String,Object>> list = tblUserMapper.selectTheCurrentUser(sql);	
		return XoadoResult.build(Integer.parseInt(BaseRetCode.CODE_SUCCESS.getRetCode()), BaseRetCode.CODE_SUCCESS.getRetMsg(),list);
	}
