...
//		定义二维码
	    ...
	    BitMatrix bitMatrix = new MultiFormatWriter().encode(json.toString(), BarcodeFormat.QR_CODE, width, height, hints);
...
//	    outputStream 转intputstream
	    ...
	    redisCache.set(QRcode, json.toString());
	    redisCache.expire(QRcode, 600);
...
//		Data没有值，从缓存中获取数据内容，收件方调用
		if(Data=="" || Data==null){
			String QRcodeValue = redisCache.get(QRcode);
			...
			redisCache.set(QRcode, jsonObject.toString());
			redisCache.expire(QRcode, 60*5);
...
//			Data有值   写入缓存  交件方调用存储数据   1获取缓存中的QRcode
	...
	public XoadoResult ThequeryScannedQrcode(String QRcode, HttpServletRequest request) {
		...
		try {
			...
			String qrcodestateValue =(String) fromObject.get("qrcodestate");
			boolean b = qrcodestateValue.equals("scanned");
			System.out.println(b);
			if(qrcodestateValue.equals("scanned")){
...
//		CODE_QRCODE_NOT_REQUEST  未被扫描
		return XoadoResult.build(Integer.parseInt(BaseRetCode.CODE_QRCODE_NOT_REQUEST.getRetCode()),BaseRetCode.CODE_QRCODE_NOT_REQUEST.getRetMsg());
	}
	/**
	 * 接收方得到业务确认收到
	 * @param QRcode
	...
	public XoadoResult TheReceiverGetData(String QRcode, HttpServletRequest request) {
		try {
			String QRcodeValue = redisCache.get(QRcode);
			if(QRcodeValue==null){
//				CODE_PROFESSIONAL_WORK_NOT_FOUND_RESOURCE  资源未被找到
			...
			redisCache.set(QRcode, fromObject.toString());
	...
	public XoadoResult HandOverThequery(String QRcode, HttpServletRequest request) {
		...
		System.out.println(qrcodestateValue);
		if(!qrcodestateValue.equals("confirmed")){
//			CODE_QRCODE_NOT_REQUEST  二维码未被访问
			return XoadoResult.build(Integer.parseInt(BaseRetCode.CODE_QRCODE_NOT_REQUEST.getRetCode()),BaseRetCode.CODE_QRCODE_NOT_REQUEST.getRetMsg());
		}
