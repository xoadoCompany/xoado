...
public class Archives implements IArchives{
	...
	public XoadoResult getarchives(Integer Page, Integer Rows, HttpServletRequest request) {
		...
		List<LinkedHashMap<String,Object>> list = tblCurrentArchivesMapper.selectByExampleTemporary(token.getUserId());
	...
	public XoadoResult Add_Aoopint_Data_Doc(String archiveId, MultipartFile[] attachmentDocument,
			HttpServletRequest request) {
//		资源未实现
		return XoadoResult.build(Integer.parseInt(BaseRetCode.CODE_FRAME_RESOURCE_NOTIMPLEMETHTATION_ERRED.getRetCode()), BaseRetCode.CODE_FRAME_RESOURCE_NOTIMPLEMETHTATION_ERRED.getRetMsg());
	}
	/**
	 * 
	 * @param archiveId
...
//			CODE_FRAME_REQUEST_NOT_TOKEN    当前用户未授权或未登录  
	...
	public XoadoResult deleteArchive(String archiveid, HttpServletRequest request) {
		...
		TblArchives tblArchives = archivesMapper.selectByPrimaryarchiveid(archiveid);
		if(tblArchives == null){
