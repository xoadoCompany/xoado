...
//		查找当前用户自己创建的档案柜
		AccessIdentity token = (AccessIdentity) request.getSession().getAttribute(XoadoConstant.XOADOTOKEN);
		String sql="SELECT box.box_id,box.box_name,box.owner_org,box.scopeofuse,box.location,box.use_state,box.close_state,box.remarks FROM tbl_boxes AS box WHERE box.deleted=0 && JSON_CONTAINS(creater,'{\"creater_id\":\""+token.getUserId()+"\"}') || JSON_CONTAINS(shareto,'{\"userid\":\""+token.getUserId()+"\"}')";
	...
	public XoadoResult remove_boex(String boxid, HttpServletRequest request) {
		...
		if(key==null){
//			CODE_PROFESSIONAL_WORK_NOT_FOUND_RESOURCE  没有此档案柜
...
//			CODE_ERROR_BREAK_THE_LAW   档案柜存在资料
...
//		CODE_ERROR_BREAK_THE_LAW  档案柜非当前用户创建
		return XoadoResult.build(Integer.parseInt(BaseRetCode.CODE_ERROR_BREAK_THE_LAW.getRetCode()),BaseRetCode.CODE_ERROR_BREAK_THE_LAW.getRetMsg());
	}
	/**
	 * 指定一个档案柜共享给一个或者多个人
...
//					CODE_PROFESSIONAL_WORK_DATA_REPEAT   共享账户中存在此用户
					 return XoadoResult.build(Integer.parseInt(BaseRetCode.CODE_PROFESSIONAL_WORK_DATA_REPEAT.getRetCode()),BaseRetCode.CODE_PROFESSIONAL_WORK_DATA_REPEAT.getRetMsg());
				 }
			}
...
//		CODE_ERROR_BREAK_THE_LAW  当前的柜子不是当前用户所创建
		return XoadoResult.build(Integer.parseInt(BaseRetCode.CODE_ERROR_BREAK_THE_LAW.getRetCode()), BaseRetCode.CODE_ERROR_BREAK_THE_LAW.getRetMsg());
	}
	/**
...
//				 CODE_PROFESSIONAL_WORK_NOT_FOUND_RESOURCE  共享账户中没有此用户
					return XoadoResult.build(Integer.parseInt(BaseRetCode.CODE_PROFESSIONAL_WORK_NOT_FOUND_RESOURCE.getRetCode()), BaseRetCode.CODE_PROFESSIONAL_WORK_NOT_FOUND_RESOURCE.getRetMsg());
			}
		}
		
//		CODE_ERROR_BREAK_THE_LAW  柜子不是当前用户所创建
		return XoadoResult.build(Integer.parseInt(BaseRetCode.CODE_ERROR_BREAK_THE_LAW.getRetCode()), BaseRetCode.CODE_ERROR_BREAK_THE_LAW.getRetMsg());
	}
	/**
	 * 	返回指定档案柜详情（创建人和被授权人）。
...
//		String sql = "SELECT BOEX.box_id,BOEX.box_name,BOEX.owner_org,BOEX.scopeofuse,BOEX.location,BOEX.remarks,BOEX.close_state,BOEX.creater,BOEX.creater_time,BOEX.shareto FROM tbl_boxes AS BOEX WHERE box_id=\""+Boexid+"\"";
		...
		if(key==null){
			return XoadoResult.build(400, "不存在");
		}
